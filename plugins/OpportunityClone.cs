...
{
    public class OpportunityClone : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            try
            {
                EntityReference newOppRef = (EntityReference)context.OutputParameters["NewOpportunityRef"];
                EntityReference clonedOppRef = (EntityReference)context.InputParameters["Target"];
                DateTime estCloseDate = (DateTime)context.InputParameters["EstCloseDate"];
                bool cloneLines = (bool)context.InputParameters["CloneLines"];
                if (cloneLines) CloneWithLines(service, tracingService, newOppRef, estCloseDate, clonedOppRef);
                else CloneWithoutLines(service, tracingService, newOppRef, estCloseDate);
            }
            #region ErrorHandling
            catch (FaultException<OrganizationServiceFault> ex)
            {
                tracingService.Trace("An error occurred in the OpportunityClone function: {0}", ex.ToString());
                tracingService.Trace("Error Details: {0}", ex.Detail.InnerFault.Message);
                throw new InvalidPluginExecutionException("An error occurred in the OpportunityClone function." + ex.Detail.InnerFault.Message.ToString());
            }
            catch (Exception ex)
            {
                tracingService.Trace("An error occurred in the OpportunityClone function: {0}", ex.ToString());
        ...
        private void CloneWithoutLines(IOrganizationService service, ITracingService tracingService, EntityReference newOppRef, DateTime estCloseDate)
        {
            try
            {
                estCloseDate = estCloseDate.AddYears(1);
            ...
            {
                tracingService.Trace("An error occurred in the CloneWithoutLines Method in the OpportunityClone function: {0}", ex.ToString());
                tracingService.Trace("Error Details: {0}", ex.Detail);
                throw new InvalidPluginExecutionException("An error occurred in the CloneWithoutLines Method in the OpportunityClone function." + ex.Detail.InnerFault.Message.ToString());
            }
            catch (Exception ex)
            {
                tracingService.Trace("An error occurred in the CloneWithoutLines in the OpportunityClone function: {0}", ex.ToString());
                throw;
            }
            #endregion
        }
        private void CloneWithLines(IOrganizationService service, ITracingService tracingService, EntityReference newOppRef, DateTime estCloseDate, EntityReference clonedOppRef)
        {
            try
            {
                estCloseDate = estCloseDate.AddYears(1);
                ...
                service.Update(uOpp);
                CreateNewOppLines(service, tracingService, newOppRef, clonedOppRef);
            }
            #region ErrorHandling
            catch (FaultException<OrganizationServiceFault> ex)
            {
                tracingService.Trace("An error occurred in the CloneWithLines Method in the OpportunityClone function: {0}", ex.ToString());
                tracingService.Trace("Error Details: {0}", ex.Detail.InnerFault.Message);
        ...
        private void CreateNewOppLines(IOrganizationService service, ITracingService tracingService, EntityReference newOppRef, EntityReference clonedOppRef)
        {
            try
            {
                #region Fetch
