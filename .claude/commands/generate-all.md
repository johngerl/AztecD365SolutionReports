Run the full pipeline: refresh SF entities, generate D365 JSON, enrich from API, refresh lastUpdate via TDS, set SF mapping flags, generate SF suggestions, extract CSV, and generate reports.

Runs Steps 1 → 2 → 3 → 4 → 5 → 6 → 7 → 8 in sequence.
...
if [ -z "$ARGUMENTS" ]; then
  ...
  echo "============================================================"
  echo ""
  echo "Step 1/8: Refreshing SF entity JSON from API..."
  echo "------------------------------------------------------------"
  python scripts/generate_sf_entity_json_from_api.py --all
  echo ""
  echo "Step 2/8: Generating D365 entity JSON from solution..."
  echo "------------------------------------------------------------"
  python scripts/generate_d365_entity_json_from_solution.py --all
  echo ""
  echo "Step 3/8: Enriching D365 entity JSON from Dataverse API..."
  echo "------------------------------------------------------------"
  python scripts/enrich_d365_entity_json_from_api.py --all
  echo ""
  echo "Step 4/8: Refreshing lastUpdate via Dataverse TDS..."
  echo "------------------------------------------------------------"
  python scripts/refresh_d365_field_lastupdates_tds.py --all
  echo ""
  echo "Step 5/8: Setting sfSuggestedMapping flags..."
  echo "------------------------------------------------------------"
  python scripts/set_d365_sf_suggested_mapping.py --all
  echo ""
  echo "Step 6/8: Generating SF suggestions (5-tier matching)..."
  echo "------------------------------------------------------------"
  python scripts/update_d365_entity_csv_mapping_with_sf_suggestions.py --all
  echo ""
  echo "Step 7/8: Generating D365 entity CSV mappings..."
  echo "------------------------------------------------------------"
  python scripts/generate_d365_entity_csv_mapping.py --all
  echo ""
  echo "Step 8/8: Generating D365 field usage reports..."
  echo "------------------------------------------------------------"
  ...
  echo "============================================================"
  echo ""
  echo "Step 1/8: Refreshing SF entity JSON from API ($SF_NAME)..."
  echo "------------------------------------------------------------"
  ...
  echo ""
  echo "Step 7/8: Generating D365 entity CSV mapping..."
  echo "------------------------------------------------------------"
  python scripts/generate_d365_entity_csv_mapping.py $ARG
  echo ""
  echo "Step 8/8: Generating D365 field usage report..."
  echo "------------------------------------------------------------"
