...
| `/update-d365-csv-with-sf [entity]` | `/update-d365-csv-with-sf account` or `/update-d365-csv-with-sf` (all) | Step 5: 5-tier SF field matching (exact, fuzzy, synonym, rule-based, AI). Writes suggestions to JSON (source of truth) and CSV. Uses DataTypeCompatibilityMatrix.md for type-aware matching. |
| `/generate-d365-csv [entity]` | `/generate-d365-csv account` or `/generate-d365-csv` (all) | Step 6: Extract mapping CSV(s) from enriched JSON. Reads sfSuggested* from JSON, preserves confirmed SF columns from CSV. |
...
Step 5:  /update-d365-csv-with-sf    JSON + SF entities + matrix   →  d365-entities/*.json (with sfSuggested*) + mapping/*.csv
Step 6:  /generate-d365-csv          d365-entities/*.json          →  mapping/*.csv (with suggestions from JSON)
