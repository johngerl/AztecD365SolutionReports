...
from existing CSVs. Clears sfSuggested* columns (regenerated by step 6).
...
def extract_mapping(source_file, output_file):
    ...
    for field in data.get("fields", []):
        ...
        # Compute mappingSuggested: staleness gate then usage/required check
        staleness = _is_stale(field.get("lastUpdate"))
        if staleness is True:
            mapping_suggested = "false"
        else:
            has_usage = any(
        ...
        # Clear sfSuggested* columns (regenerated by step 6)
