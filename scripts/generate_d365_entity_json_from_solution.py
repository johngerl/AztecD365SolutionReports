...
and writes enriched entity JSON with entity-level sections and per-field references.
...
def parse_field_definitions(entity_el):
    ...
    for attr in attributes.findall("attribute"):
        ...
        fields.append({
            ...
            'valid_for_create': text_of(attr, 'ValidForCreateApi', '0') == '1',
            'valid_for_read': text_of(attr, 'ValidForReadApi', '0') == '1',
            'valid_for_update': text_of(attr, 'ValidForUpdateApi', '0') == '1',
            'is_searchable': text_of(attr, 'IsSearchable', '0') == '1',
            'is_filterable': text_of(attr, 'IsFilterable', '0') == '1',
            'is_sortable': text_of(attr, 'IsSortableEnabled', '0') == '1',
            'is_global_filter_enabled': text_of(attr, 'IsGlobalFilterEnabled', '0') == '1',
            'is_logical': text_of(attr, 'IsLogical', '0') == '1',
            'is_retrievable': text_of(attr, 'IsRetrievable', '0') == '1',
            'is_data_source_secret': text_of(attr, 'IsDataSourceSecret', '0') == '1',
...
_DATA_DICT_KEYS = frozenset({'fieldRefs', 'funcFields'})


def convert_keys_to_camel(obj, _parent_key=None):
    ...
    if isinstance(obj, (dict, defaultdict)):
        if _parent_key in _DATA_DICT_KEYS:
            ...
            return {k: convert_keys_to_camel(v) for k, v in obj.items()}
        return {
            snake_to_camel(k): convert_keys_to_camel(v, _parent_key=snake_to_camel(k))
            for k, v in obj.items()
        }
    if isinstance(obj, list):
        return [convert_keys_to_camel(item, _parent_key=_parent_key) for item in obj]
...
def enrich_entity(entity_name, root, property_to_field, class_to_entity):
    ...
    known_fields = set(f['schema_name'] for f in field_defs)
    ...
    # --- Field augmentation: add stub entries for fields referenced but not defined ---
    all_referenced = set()
    ...
    for field_name in missing:
        field_defs.append({
            ...
            'is_filterable': False,
            'is_sortable': False,
            'is_global_filter_enabled': False,
            'is_logical': False,
    ...
    for fdef in field_defs:
        ...
        field_out = {
            ...
            'displayMask': fdef.get('display_mask', ''),
            'validForCreate': fdef.get('valid_for_create', False),
            'validForRead': fdef.get('valid_for_read', False),
            'validForUpdate': fdef.get('valid_for_update', False),
            'isSearchable': fdef.get('is_searchable', False),
            'isFilterable': fdef.get('is_filterable', False),
            'isSortable': fdef.get('is_sortable', False),
            'isGlobalFilterEnabled': fdef.get('is_global_filter_enabled', False),
            'isLogical': fdef.get('is_logical', False),
    ...
    # Entity-level sections (full parse outputs, camelCase)
    sections = {
        'forms': convert_keys_to_camel(forms),
        'views': convert_keys_to_camel(views),
        'chartVisualizations': convert_keys_to_camel(chart_visualizations),
        'reports': convert_keys_to_camel(reports),
        'dashboards': convert_keys_to_camel(dashboards),
        'workflows': convert_keys_to_camel(workflows),
        'javaScript': convert_keys_to_camel(js_files),
        'formulas': convert_keys_to_camel(formulas),
        'plugins': convert_keys_to_camel(plugin_refs),
        'pcfControls': convert_keys_to_camel(pcf_controls),
        'relationships': convert_keys_to_camel(relationships),
        'ribbon': convert_keys_to_camel(ribbon),
        'conflicts': convert_keys_to_camel(conflicts),
    }
